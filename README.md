...
4. **Fill `.env`.** Never share this file or paste the token anywhere.
   ...
   | `VISION_ENGINE` | no | `claude_code` | `claude_code` reads screenshots with Claude Code on this PC (your Claude plan); `api` uses Anthropic API credit; `agy` uses Gemini through agy and the local OCR proxy; `cmdc` uses the Command Code CLI on this PC (your Command Code plan) — see *Screenshots* |
   ...
   | `CMDC_PATH` | no | found automatically | `cmdc` engine: path to `cmdc.cmd` if it is not on `PATH` or in `%APPDATA%\npm` |
   | `CMDC_MODEL` | no | `qwen/qwen3.8-27b` | `cmdc` engine: model that can read images (`cmdc --list-models`) |
...
| `/list` | Your parcels |
| | *The command menu shows only `/start`, `/help`, `/list` and `/location`. Every other command below still works when typed — each one is also a button on a parcel's card or under `/list`. `/help` follows the same rule and documents only `/track` and `/phone`, the two with no button equivalent.* |
...
- **Where it is and how far:** every row in `/list` says where the parcel last was and how far that is from your saved area (`📦 Kiện hàng đã tới … · cách bạn ~N km`), with the carrier's own status underneath. Orders still on their way come before finished ones, and a button under the list cycles the order between 📊 Theo trạng thái, 📍 Gần bạn nhất, 🚚 Theo hãng and 🔤 Theo tên — the list opens in 📊 Theo trạng thái. The status order groups the list by the stage each parcel has reached — on the way, nearly there, still being identified, gone quiet, finished — newest movement first inside each group, and its headings replace the two-part split. Distances need `/location`; a hub nobody has looked up yet shows its name without a distance.
- **Buttons:** update messages and add replies carry ✏️ Đổi tên (rename), 📜 Hành trình (history), 🔄 Kiểm tra (rebuild this parcel from a fresh check with the latest scripts, once every 5 minutes), 🗑 Xóa (remove after confirming), 📤 Chia sẻ (share link), 🗺 Bản đồ (map picture, see *Maps and distance*) and 🔗 Tra cứu ↗ (tracking page). Taps edit the same message. `/list` and digests show numbered buttons that open a parcel card; `/list` shows 5 parcels per page with ⬅️/➡️.
- **No repeat updates:** a scan is announced once. A source that re-sends the same scan in different words between checks — 17TRACK does this on cross-border parcels — is not stored again and does not produce a second message.
...
**Reading screenshots with cmdc (Command Code) instead.** Set `VISION_ENGINE=cmdc` in `.env` and restart the bot. No API key is involved: the bot uses the `cmdc` login already on this PC (`cmdc whoami` should print your account), and each screenshot counts toward your Command Code plan. cmdc cannot take an image on its command line, so the bot writes the screenshot into a new temporary folder and runs `cmdc -p` there, telling it to open that file with its own `read_file` tool. The run is asked for JSON on the model in `CMDC_MODEL` (a model that accepts images — see `cmdc --list-models`), with skills and taste onboarding switched off, and it stays read-only: the bot never passes `--yolo`, so text inside a screenshot cannot make it write files or run commands. Like `agy`, a code that matches no carrier or has the wrong length for its carrier makes the bot read the screenshot once more and keep the better read. Each screenshot carries roughly 30k tokens of agent-prompt overhead on top of the image and takes about 10–20 seconds. `deepseek/deepseek-v4-flash-vision-exp` is the cheapest image model but added a digit to a 17-character SPX code in testing; `qwen/qwen3.8-27b` and `moonshotai/kimi-k2.6` read it correctly.
...
The bot sends a sticker now and then, silently, just before an update message — chosen by the parcel's **delivery status** rather than its carrier: `moving` (on the way), `near` (out for delivery), `delivered`, `returned`. Each one carries that status in Vietnamese, using the same wording `/sticker` lists. Telegram bots cannot create a sticker pack, but they can upload a `.webp` and send it as a sticker on its own, so the first start uploads each file in `src/vn_parcel_bot/assets/status/`, keeps the `file_id` it gets back and deletes the message it went out on; a later start uploads again only what has been redrawn since (the drawing's hash is kept in `sticker-art:<status>`). They are drawn, not sourced: `python scripts/make_status_stickers.py` redraws them, and a new status needs an entry in `STICKER_STATUSES` (`services/stickers.py`) and its Vietnamese meaning in `texts.STICKER_STATUS_TEXT`.

Two things replace that shipped picture. **Reply to `/sticker <trạng thái>` with a photo** and the bot writes the status onto it — that image becomes the cover for that status, so every later sticker for it looks like yours. And **a parcel added from a screenshot keeps that screenshot**: its own stickers carry the parcel's picture (`data/covers/<id>.webp`, deleted when the parcel is) rather than the badge, falling back to the badge when there is none. Both re-upload only when the drawing or the wording actually changes.
