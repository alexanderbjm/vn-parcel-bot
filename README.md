...
- **Where it is and how far:** every row in `/list` says where the parcel last was and how far that is from your saved area (`📦 Kiện hàng đã tới … · cách bạn ~N km`), with the carrier's own status underneath. Orders still on their way come before finished ones, and a button under the list switches the order between 📍 Gần bạn nhất, 🚚 Theo hãng, 🔤 Theo tên and 📊 Theo trạng thái. The status order groups the list by the stage each parcel has reached — on the way, nearly there, still being identified, gone quiet, finished — newest movement first inside each group, and its headings replace the two-part split. Distances need `/location`; a hub nobody has looked up yet shows its name without a distance.
