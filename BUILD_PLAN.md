...
Version 3.2 · 2026-09-18 · Status: v1 built on branch main; live verification in progress
...
## Changes in 3.2 (2026-09-18)

- **A cover's words keep clear of the message time** (§9.13): a chat draws its message timestamp over the bottom-right of a sticker, and the band on a cover sat right at the foot, so the time landed on the last line and hid it — confirmed on a live screenshot, where «12:22» sat on top of a sticker's label. The band now stops `BAND_FOOT` (54 px) above the foot, and a test asserts nothing white is drawn below that line. The shipped badge was never affected: its words are centred, far above the corner. What the admin draws outside the repo (the cat set, installed through `/sticker`) is unaffected by this constant.
...
- **Stickers follow the delivery status, not the carrier** (§4.1, §6.3, §9.13): the five carrier logos are gone. One drawn sticker per status — `moving`, `near`, `delivered`, `returned`, in `src/vn_parcel_bot/assets/status/<status>.webp` and made by `scripts/make_status_stickers.py` — is sent before an update, chosen by `sticker_status(state, progress)` (in transit at 80% or more is `near`). `/sticker` now maps a sticker to a status (`/sticker <trạng thái> [off]`), the meta keys are `sticker:<status>` and `sticker-auto:<status>`, and with the carrier stickers went the one-time migration that dropped a carrier sticker mapped before the icons existed. Rows left under the old carrier keys are no longer read.
...
- **17TRACK as a last resort** (§5.1, §10.1): a parcel whose own carriers return no data is looked up on 17TRACK without naming a carrier, so 17TRACK identifies it (STO, YT, China Post…). Registering costs one quota, so it happens once per parcel and the attempt is stored in `meta` as `17track-tried:<id>`; the events then follow the normal update path. Chinese descriptions are translated by `carriers/translate_cn.py`, which also drops courier names, phone numbers and advertising. `SEVENTEEN_FALLBACK=false` turns it off.
...
- **Checks timed by delivery stage** (§6.1, §6.4, §9.2): the `poll` job ticks every minute and checks the parcels whose `next_check_at` has passed. After a check a parcel waits 3 min when in transit with progress ≥ 80 (delivery hub, out for delivery), 10 min when otherwise in transit, and `POLL_INTERVAL_MINUTES` (20) while it has no data; failures back off from that interval. Idle ticks log nothing.
- **Check all and match carriers again** (§4, §9.9, §9.12): `/check` and a new `🔄 Kiểm tra tất cả` button under `/list` (callback `r:<page>`) first run `ParcelService.redetect_carriers` (an active parcel whose detected tracked carriers no longer include its carrier, or differ from its candidates, gets them as new candidates with a zero failure count via `Repository.set_candidates`), then check all of the user's active parcels now. `CHECK_DONE` adds `CHECK_REDETECTED` when carriers changed; the button redraws the list under the summary. One `RECHECK_COOLDOWN` (2 min) per user covers both.
- **Tracking code always shown** (§9.9, §17): `parcel_title` is the label, ` · ` and the blurred code (just the blurred code without a label); `masked_title` is gone. `/label`, `/remove` and card prompts show the blurred full code instead of a masked one, and a replied bot message that still shows the code in clear is edited to blur it.
- **`/hozk`** (admin): lists the admin commands (`ADMIN_HELP`). `ADMIN_COMMANDS` (hozk, users, allow, revoke, health, sticker) are added to the admin's own command menu through `BotCommandScopeChat`.
...
- **SF Express** (§5.1, §5.2 rule 12): `SF` + 13 digits is recognised as a thirteenth, link-only carrier instead of an unknown code. Its official query pages are JavaScript apps and no open endpoint was found, so it is not polled and is never stored (the `carrier` CHECK is unchanged).
- **Logs for silent photo results**: the photo handler logs INFO `"photo read codes=%d order_ids=%d product=%s"` (counts and yes/no only) for every successful read, and `ParcelService.add` logs INFO `"duplicate code user=%s code=%s"` with the masked code. Before this, a photo whose code was already tracked, or that held only an order number or nothing, left no log line.
- BEST Express VN's track page was re-checked on 2026-09-14 and shows a slider captcha; it stays link-only.
...
BAND_MARGIN = 18
BAND_HEIGHT = 132
BAND_FOOT = 54                               # lift the band clear of the message time
...
- Each sticker writes the status in **Vietnamese**, uppercase, from `texts.STICKER_STATUS_TEXT` — the same wording `/sticker` lists, so the drawing and the command cannot drift apart. `sticker_art.py` draws it for both the shipped art and a cover: a coloured badge, or a band across the foot of a photo, with the words at the largest size that fits — one line while that stays at least `SINGLE_LINE_MIN_SIZE` and otherwise the two lines that balance best. The band stops `BAND_FOOT` above the foot of the sticker, because a chat draws its message timestamp over the bottom-right corner and at `BAND_MARGIN` the words reached down into it and the time hid them; the badge needs no such room, its words are centred. `scripts/make_status_stickers.py` writes the four shipped files; only a system font is read and only its rendered output ships, so the art is licence-free and can be recoloured or reworded by rerunning it.
...
- `test_catalog_has_thirteen_carriers_in_table_order`.
...
9. Log INFO `"parcel added user=%s carrier=%s code=%s state=%s"` with `mask_code(code)` and `parcel.carrier or "auto"`; log INFO `"link-only code user=%s carriers=%s code=%s"`; log INFO `"duplicate code user=%s code=%s"` with `mask_code(code)` when the parcel already exists.
