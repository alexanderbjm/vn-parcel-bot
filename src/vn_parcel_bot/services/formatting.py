...
# Status leads: /list opens grouped by the stage each parcel has reached, which answers "where
# is my stuff" before "what is nearest". The button walks the rest in this order and wraps.
SORT_MODES = ("s", "n", "c", "a")
DEFAULT_SORT = SORT_MODES[0]


def next_sort(sort: str) -> str:
    """The mode the sort button moves on to: status, then nearest, then carrier, then A-Z."""
...
def sort_parcels(
    ...
    distances: Mapping[int, float] | None = None,
) -> list[Parcel]:
